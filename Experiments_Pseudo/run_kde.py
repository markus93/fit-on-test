...
class KDE_estimator():
    ...
    def __init__(self, p, y, c=None, calibration_function=None):
        ...
        if c is not None and calibration_function is not None:
...
# Suited for Cifar-5m tests
def run_kde(p, y, c_list, cal_method = "TempS", data_name = "resnet", n_data = 10000, seed = 1, data_path = ".", data_part = -1):

    model = KDE_estimator(p, y)
    
    c_hat = model.c_hat    
    ECE_integral = model.integral_ece_abs, 
    ECE_integral_square = model.integral_ece_sq
    ECE = model.pointwise_ece_abs
    ECE_square = model.pointwise_ece_sq
    ...
    data_row = {"seed": seed,
            ...
            "binning": "KDE",
            "n_bins": -1,
            "c_hat_distance_p": ECE,
            "c_hat_distance_p_square": ECE_square,
    ...
    data_row2 = {"seed": seed,
        ...
        "calibration_function": cal_method,
        "data_name": data_name,
        "binning": "KDE_integral",
        "n_bins": -1,
        "c_hat_distance_p": ECE_integral,
        "c_hat_distance_p_square": ECE_integral_square,
        "c_hat_distance_c": c_hat_distance_c,
        "c_hat_distance_c_square": c_hat_distance_c_square,
        "p_distance_c": p_distance_c,
        "p_distance_c_square": p_distance_c_square,
    ...
    df = pd.DataFrame([data_row, data_row2])
    df.to_pickle(join(data_path, f"KDE_seed_{seed}_{n_data}_{cal_method}_{data_name}_dp_{data_part}.pkl"), protocol = 4)
